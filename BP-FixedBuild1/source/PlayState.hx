...
import Shaders.BlockedGlitchShader2;
import Shaders.BlockedGlitchEffect2;
...
class PlayState extends MusicBeatState
{
	...
	public static var blockedShaderAlt:BlockedGlitchEffect2;
	...
	var dontDarkenChar:Array<String> = ['bambi-god', 'bambi-god2d', 'bambi-god2d-24fps', 'baiburgascend', '404'];
	...
	var dontMiddle:Array<String> = ['cheating', 'disposition-old', 'despair', 'devastation-fanmade']; // dont middlescroll (this makes the arrows not go offscreen with middlescroll unless u manage to force the game to 999 fps LOL)
	...
	public var BoyfriendWindowCenterPos:FlxPoint = new FlxPoint(0,0);
	...
	var pressingKey5Global:Bool = false; //god
	var key5 = FlxG.keys.justPressed.SPACE;
	...
	public var ogDefaultCamZoom:Float = 1.05;
	...
	var TURNITTHEFUCKON = true; //pretty stupid way to deal with the disposition cameras but whatever since defaultcamzoom won't cooperate with me
	...
	override public function create()
	{
		...
		#if windows
		shader_chromatic_abberation = new ChromaticAberrationEffect(0.0001); // i think this one was from psych itself?
		...
		if(SONG.stage == null || SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'maze' | 'old-maze' | 'beta-maze':
					curStage = 'farmSunset';
				case 'splitathon' | 'old-splitathon' | 'mealie' | 'supplanted' | 'screwed' | 'supplanted-old':
					curStage = 'farmNight';
				...
				case 'exploitation':
					curStage = 'desktop';	
				case 'disposition' | 'disposition alt' | 'disposition-old' | 'disposition_but_awesome':
		...
		case 'bambersHell':
			{
				...
				cloudsH = new BGSprite('bambi/purgatory/scaryclouds', -400, 250, 1.2, 1.2);
		...
		case 'bobuRsod':
				...
				bgrsod = new FlxSprite(-600, -200).loadGraphic(Paths.image('dave/KERNEL_DATA_INPAGE_ERROR'));
		...
		case '3dBembos':
			{
				...
				ourple = new DepthSprite('bambi/poipBG', -1500, -800, 0.2, 0.2);
				ourple.antialiasing = true;
				ourple.depth = 0.2;
				ourple.defaultScale = 0.7;
				ourple.screenCenter();
				ourple.active = true;
				add(ourple);
				...
				phones = new DepthSprite('bambi/phones', -1500, -300, 0.6, 0.6);
		...
		case 'double':
				...
				ourple = new DepthSprite('bambi/POPI_AND_CRUSTI_OMGGGGG', -1500, -800, 0.2, 0.2);
				ourple.antialiasing = true;
				ourple.depth = 0.2;
				ourple.defaultScale = 0.7;
				ourple.screenCenter();
				...
				phones = new DepthSprite('bambi/UWU', -1500, -300, 0.6, 0.6);
				phones.antialiasing = false;
				phones.depth = 0.6;
				phones.screenCenter(X);
				phones.x -= 100;
		...
		case 'farmDay':
			{
				...
				var sky:BGSprite = new BGSprite('bambi/custom/day/sky_day', -200, -300, 0, 0);
				//sky.scale.set(0.5,0.5);
				add(sky);

				var hills:BGSprite = new BGSprite('bambi/custom/sunset/hills', -1500, -1600, 0.2, 0.2);
				hills.scale.set(0.5,0.5);
				add(hills);

				var farm:BGSprite = new BGSprite('bambi/custom/sunset/farm', -1300, -1500, 0.65, 0.65);
				farm.scale.set(0.6,0.6);
				add(farm);

				var foreground:BGSprite = new BGSprite('bambi/custom/sunset/foreground', -1350, -1550, 1, 1);
				foreground.scale.set(0.75,0.75);
				add(foreground);

				colorFilter = new BGSprite(null, -800, -400, 0, 0);
				colorFilter.makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), 0xFFFFFFFF);
		...
		case 'farmSunset':
			{
				...
				var sky:BGSprite = new BGSprite('bambi/custom/sunset/sky_evening', -200, -300, 0, 0);
				//sky.scale.set(0.5,0.5);
				add(sky);
		...
		case 'farmNight':
			{
				...
				*/
				...
				var sky:BGSprite = new BGSprite('bambi/custom/night/skye_gapple_reference', -1500, -1200, 0, 0);
				sky.scale.set(0.5,0.5);
				add(sky);

				var hills:BGSprite = new BGSprite('bambi/custom/night/hills', -1500, -1600, 0.2, 0.2);
				hills.scale.set(0.5,0.5);
				add(hills);

				var farm:BGSprite = new BGSprite('bambi/custom/night/farm', -1300, -1500, 0.65, 0.65);
				farm.scale.set(0.6,0.6);
				add(farm);

				var foreground:BGSprite = new BGSprite('bambi/custom/night/foreground', -1350, -1550, 1, 1);
				foreground.scale.set(0.75,0.75);
				add(foreground);
	        }
		...
		case 'backyard':
		{
			...
			var skyBG:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('backgrounds/shared/sky'));
			add(skyBG);

			var hills:FlxSprite = new FlxSprite(-1330, -432).loadGraphic(Paths.image('backgrounds/backyard/hills', 'shared'));
			add(hills);

			var grass:FlxSprite = new FlxSprite(-800, 150).loadGraphic(Paths.image('backgrounds/backyard/supergrass', 'shared'));
			grass.scale.set(1.2, 1.2);
			add(grass);

			var gates:FlxSprite = new FlxSprite(564, -33).loadGraphic(Paths.image('backgrounds/backyard/gates', 'shared'));
			add(gates);
				
			var bear:FlxSprite = new FlxSprite(-1035, -710).loadGraphic(Paths.image('backgrounds/backyard/bearDude', 'shared'));
			add(bear);

			var house:FlxSprite = new FlxSprite(-1025, -323).loadGraphic(Paths.image('backgrounds/backyard/house', 'shared'));
			add(house);

			var grill:FlxSprite = new FlxSprite(-489, 452).loadGraphic(Paths.image('backgrounds/backyard/grill', 'shared'));
			add(grill);
		}
			case 'stage': //Week 1
		...
		switch(curStage)
		{
			...
			case 'farmNight':
				var filter:BGSprite = new BGSprite('generalBGshit/nightGradient', -500, -300, 0, 0);
				filter.screenCenter();
				...
				if (SONG.song.toLowerCase() == 'reality breaking')
					glowPrefix = 'rbGlow';
				glow = new BGSprite('generalBGshit/'+glowPrefix, -500, -300, 0, 0);
				glow.screenCenter();
				//glow.x += 25;
				//glow.y += 25;
		...
		{
			case 'bambi-scaryooo' | 'bambi-god' | 'bambi-god2d' | 'bambi-hell' | 'expunged' | '404':
		...
		{
			case 'roundabout' | 'roundabout old' | 'upheaval-teaser' | 'reheated' | 'lacuna' | 'antagonism-poip-part' | 'dethroned' | 'crimson corridor' | 'demise pt 1' | 'demise pt 2' | 'demise-pt-1' | 'demise-pt-2':
		...
		rsod = new FlxSprite(0, 0).loadGraphic(Paths.image('generalBGshit/rsod'));
		if(SONG.song.toLowerCase() == "rsod-old") 
			add(rsod);
		rsod.visible = false;
		rsod.cameras = [camHUD];

		notResponding = new FlxSprite(0, 0).loadGraphic(Paths.image('generalBGshit/nr'));
		...
		if(ClientPrefs.uiStyle == 'Kade Engine') {
			...
			+ " | " + engineName + MainMenuState.psychEngineVersion, 14);
		...
		switch (SONG.song.toLowerCase())
		{
            ...
            case 'reality breaking' | 'technology' | 'body destroyer' | 'face-destroyer' | 'annihilation' :
				composersWatermark = 'Pyramix';
			// add randomness songs here 
		    case 'shattered' | 'fallowed' | 'upheaval wip' | 'tyranny':
			...
			case 'computer' | 'crimson corridor' | 'demise pt 1' | 'demise pt 2' | 'double act':
			...
			case 'antagonism-poip-part':
				composersWatermark = 'GambitGourmet/ikaens';
			case 'bambi-bass':
				composersWatermark = 'Biddle3';
			case 'devastation':
				composersWatermark = 'Hortas, Pyramix';
			case 'inflorescence':
				composersWatermark = 'Hortas';
			case 'devastation-fanmade':
				composersWatermark = 'Ayop';
			case 'disposition' | 'disposition alt':
				composersWatermark = 'EpicRandomness11 & ShredBoi';
			case 'disposition_but_awesome' | 'roundabout' | 'roundabout old' | 'antagonism' |'antagonism-11-minutes' | 'upheaval' | 'antagonism-test' | 'rebound' | 'rebound old' | 'deploration':
			...
			case "beefin'":
		    	composersWatermark = 'Villezen'; // who will make v2
			case 'dishonored':
				composersWatermark = 'ZlixGH';
			// old songs
			case "beefin'-old'":
				composersWatermark = 'Cynda';
			case 'disposition old':
				composersWatermark = 'ShredBoi';
			// fdsgujhosfdjohfsdjgn
			default:
				composersWatermark = 'Unknown Composer';
		}
		...
		composersBG = new FlxSprite().makeGraphic(Std.int((composersText.textField.width + ballsText.width) + 20), Std.int((composersText.height / 2) + (ballsText.height / 2)) + 30,
		...
		devTxt = new FlxText(0, 0, FlxG.width, "- DEVELOPER BUILD -", 20);
		devTxt.setFormat(Paths.font("comic.ttf"), 40, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		devTxt.scrollFactor.set();
		devTxt.borderSize = 4;
		devTxt.antialiasing = ClientPrefs.globalAntialiasing;
		devTxt.alpha = 0.5;
		devTxt.screenCenter();
		devTxt.cameras = [camOther];
		add(devTxt);
		...
		tutorialTxt = new FlxText(FlxG.width, "Hit the Restart Notes.", 20);
		if (!ClientPrefs.downScroll)
		...
		blockedShaderAlt = new BlockedGlitchEffect2();
		...
		if (isStoryMode || isPurStoryMode || ClientPrefs.freeplayCuts && !seenCutscene)
		{
			switch (daSong)
			{
				case 'senpai' | 'roses' | 'thorns' | 'polygonized' | 'furiosity' | 'cheating' | 'unfairness' | 'house' | 'insanity' | 'blocked' | 'corn-theft' :
					...
					schoolIntro(doof);
						...
						case 'splitathon' | 'shattered' | 'supplanted' | 'reality-breaking' | 'rebound' | 'disposition' | 'upheaval' | 'disposition alt':
							dialogBullshitStart();
  ...
  public function startVideo(name:String)
	...
	function cacheCountdown()
	{
		...
		if (SONG.song.toLowerCase() == 'deploration' || SONG.song.toLowerCase() == 'dishonored' || SONG.song.toLowerCase() == 'exploitation') {
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				if (curSong == 'Roundabout' || curSong == 'Roundabout Old')
				{
					swagCounter = 4;
				}
	...
	function startSong():Void
	{
		...
		for (dicknballs in [composersText, ballsText, composersBG]) {
			if (dicknballs != null) {
		...
		switch (SONG.song.toLowerCase())
		{
			case 'exploitation':
				...
				Application.current.window.setIcon(lime.graphics.Image.fromFile("art/appIcons/aaaa.png"));
	...
	if (SONG.song.toLowerCase() == 'disposition old')
			{
				if (ClientPrefs.laneunderlay){
	...
	if(SONG.song.toLowerCase() == 'rebound' || SONG.song.toLowerCase() == 'rebound old') 
	{
		for(str in playerStrums)
		{
			str.angle = 15*Math.cos((elapsedtime*2)+str.ID*2);
	...
	if(SONG.song.toLowerCase() == 'despair')
	{
		...
		blockedShaderAlt.set_Enabled(glitchCam);
	...
	if (SONG.stage == 'ourple' || SONG.stage == 'double') {
	...
	switch (SONG.song.toLowerCase())
	{
		...
		case 'insanity':
			switch (curStep)
			{
				...
				case 708:
					defaultCamZoom = 0.8;
					//dad.animation.play('um', true); // if this doesnt work istg im doing this in lua
				case 1176:
					FlxG.sound.play(Paths.sound('static'), 0.1);
					curbg.visible = true;
				case 1180:
		...
		if (blockedShaderAlt != null)
		{
			blockedShaderAlt.update(elapsed);
		}
		...
		if (ClientPrefs.iconBounce == 'Psych Engine' && !laggingRSOD) {
		...
		if (ClientPrefs.iconBounce == 'Vanilla FNF' && !laggingRSOD) {
		...
		if (ClientPrefs.iconBounce == 'Fixed Build' && !laggingRSOD) {
		...
		if (ClientPrefs.iconBounce == 'Dave Engine' && !laggingRSOD) {
			if (!iconP1.isAnim) {
		...
		if (ClientPrefs.iconBounce == 'Golden Apple' && !laggingRSOD) {
		...
		if (ClientPrefs.iconBounce == 'Custom Icon Bounce' && !laggingRSOD) {
		...
		if (camZooming)
		{
			if(allowGamecamToZoom)
				FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom, FlxG.camera.zoom, CoolUtil.boundTo(camZoomSpeed - (elapsed * 10), 0, 1));
			if(allowHUDcamToZoom)
				camHUD.zoom = FlxMath.lerp(defaultHUDZoom, camHUD.zoom, CoolUtil.boundTo(hudZoomSpeed - (elapsed * 10), 0, 1));
		}
		...
		switch(eventName) {
			...
			case 'Toggle Blocked Glitch':
				...
				switch (newvariable)
				{
					...
					case 1: 
						glitchCam = true;
						camHUD.setFilters([new ShaderFilter(blockedShader.shader)]); //screw glitchcam im doing this instead
						defaultCamZoom += 0.1;
						//FlxTween.tween(blackScreendeez, {alpha: 0.35}, Conductor.stepCrochet / 500);
				}
	...
	public function moveCamera(isDad:Bool)
	{
		...
		if(isDad)
		{
			...
			if (SONG.song.toLowerCase() == 'rebound' || SONG.song.toLowerCase() == 'rebound old' || SONG.song.toLowerCase() == 'disposition' || SONG.song.toLowerCase() == 'disposition alt' || SONG.song.toLowerCase() == 'disposition old' || SONG.song.toLowerCase() == 'dispositon_but_awesome' || SONG.song.toLowerCase() == 'upheaval' || SONG.song.toLowerCase() == 'upheaval-teaser' || SONG.song.toLowerCase() == 'upheaval wip')
			{
				if (TURNITTHEFUCKON)
					defaultCamZoom = 0.55;
			}

			if (SONG.song.toLowerCase() == 'rsod' || SONG.song.toLowerCase() == 'rsod-old') {
		...
		if(!isDad && !isboyfriend2)
		{
			...
			if (SONG.song.toLowerCase() == 'rebound' || SONG.song.toLowerCase() == 'rebound old' || SONG.song.toLowerCase() == 'disposition' || SONG.song.toLowerCase() == 'disposition old' || SONG.song.toLowerCase() == 'disposition alt' || SONG.song.toLowerCase() == 'disposition_but_awesome' || SONG.song.toLowerCase() == 'upheaval' || SONG.song.toLowerCase() == 'upheaval-teaser' || SONG.song.toLowerCase() == 'upheaval wip')
			{
				if (TURNITTHEFUCKON)
					defaultCamZoom = 0.7;
			}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(badaiTime)
		{
			...
			if(note.noteType == 'Alt Strum') {
				player3.playAnim(animToPlay + altAnim, true);
				player3.holdTimer = 0;
			}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				var daAlt = '';
				//if(note.noteType == 'Alt Animation' || note.noteType == 'Phone Poses Note') daAlt = '-alt';
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'blocked':
				switch (curStep)
				{
					case 128:
						...
						subtitleManager.addSubtitle("I'm block you...", 0.02, 1);
					case 165:
			...
			case 'maze':
				switch (curStep)
				{
					...
					case 847:
						subtitleManager.addSubtitle('Anything you want', 0.02, 0.5);
					case 856:
						subtitleManager.addSubtitle('You play me?', 0.02, 1);
					case 867:
						subtitleManager.addSubtitle('Sure.', 0.02, 1, {subtitleSize: 40});
					case 879:
						subtitleManager.addSubtitle('What you want to play?', 0.02, 1);
					case 890:
						subtitleManager.addSubtitle('You are bully me?', 0.02, 1);
					case 902:
						subtitleManager.addSubtitle('Never come back again.', 0.02, 1, {subtitleSize: 60});
						makeInvisibleNotes(false);
					case 908:
			...
			case 'reality breaking' | 'reality-breaking':
				switch (curStep)
				{
					case 256:
						camZooming = true;
					...
					case 2048:
						...
						for (spr in [healthBar, healthBarBG, healthBarOverlay, iconP1, iconP2, scoreTxt]) {
			...
			case 'disposition' | 'disposition alt':
				...
				{
					case 496:
						TURNITTHEFUCKON = false;
						defaultCamZoom += 0.135;
					case 504:
						defaultCamZoom += 0.185;
					case 512:
						TURNITTHEFUCKON = true;
						defaultCamZoom -= 0.32;
					case 752:
						TURNITTHEFUCKON = false;
			...
			case 'rsod-old': // i should probably organize this one jesus
				switch (curStep)
				{
					...
					case 368 | 1167 | 2192:
						...
						camZooming = false;
						bgrsod.active = false;
						openfl.Lib.application.window.title = "Friday Night Funkin': Bambi's Purgatory: Testing Grounds (Not Responding)";
                    case 400 | 1200 | 2224:
				    	rsod.visible = true;
						showonlystrums();
						camHUD.shake(0.0055, 0.35);
					...
					case 416 | 1216 | 2240:
						...
						bgrsod.active = true;
						crap();
						openfl.Lib.application.window.title = "Friday Night Funkin': Bambi's Purgatory: Testing Grounds";
	...
	override function beatHit()
	{
		...
		if (camZooming && autoCamZoom && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && !camZoomSnap && !camZoomHalfSnap && curBeat % 4 == 0 && !laggingRSOD)
		{
			if(allowGamecamToZoom && !doingSMzoom) FlxG.camera.zoom += 0.015 * camZoomingMult;
			if(allowHUDcamToZoom) camHUD.zoom += 0.03 * camZoomingMult;
		}

		if (camZoomSnap && !laggingRSOD) {
		...
		if (camZoomHalfSnap && !laggingRSOD) {
		...
		if (ClientPrefs.iconBounce == 'Vanilla FNF' && !laggingRSOD || ClientPrefs.iconBounce == 'Fixed Build' && !laggingRSOD) {
		...
		if (ClientPrefs.iconBounce == 'Dave Engine' && !laggingRSOD) {
			if(!iconP1.isAnim) iconP1.setGraphicSize(Std.int(iconP1.width + (50 * (funny + 0.1))),Std.int(iconP1.height - (25 * funny)));
			iconP1.updateHitbox();

	    	if(!iconP2.isAnim) iconP2.setGraphicSize(Std.int(iconP2.width + (50 * ((2 - funny) + 0.1))),Std.int(iconP2.height - (25 * ((2 - funny) + 0.1))));
	     	iconP2.updateHitbox();
		}
		...
		if (gf != null && curBeat % Math.round(gfSpeed * gf.danceEveryNumBeats) == 0 && gf.animation.curAnim != null && !gf.animation.curAnim.name.startsWith("sing") && !gf.stunned && !laggingRSOD)
		{
			gf.dance();
		}
		//health icon bounce but epic
		if (ClientPrefs.iconBounce == 'Golden Apple' || ClientPrefs.iconBounce == 'Fixed Build' && !laggingRSOD) {
		...
		if (curBeat % boyfriend.danceEveryNumBeats == 0 && boyfriend.animation.curAnim != null && !boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.stunned && !laggingRSOD)
			{
				boyfriend.dance();
		    	if(UsingNewCam) bfSingYeah = false;
		...
		if (boyfriend2 != null){
			if (curBeat % boyfriend2.danceEveryNumBeats == 0 && boyfriend2.animation.curAnim != null && !boyfriend2.animation.curAnim.name.startsWith('sing') && !boyfriend2.stunned && !laggingRSOD)
				{
					boyfriend2.dance();
		    		if(UsingNewCam) bfSingYeah = false;
					//boyfriend.playAnim('idle', true);
		...
		if(curBeat % 2 == 0)
		{
			if (curBeat % dad.danceEveryNumBeats == 0 && dad.animation.curAnim != null && !dad.animation.curAnim.name.startsWith('sing') && !dad.stunned && !laggingRSOD)
			{
				dad.dance();
				camFollowX = 0;
				camFollowY = 0;
			...
			if (curBeat % player3.danceEveryNumBeats == 0 && player3.animation.curAnim != null && !player3.animation.curAnim.name.startsWith('sing') && !player3.stunned && !laggingRSOD)
			{
				player3.dance();
	...
	override function sectionHit()
	{
		...
		if (SONG.notes[curSection] != null)
		{
			...
			if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && !laggingRSOD)
			{
