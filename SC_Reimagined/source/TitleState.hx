...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		logoBl.animation.addByPrefix('bump', 'logoBumpin', 45, false);
		...
		titlestatebg.velocity.set(110, 0); //idk i gotta put 1 as the y otherwise it won't a p p e a r 
		...
		reimaginedTxt = new FlxSprite(575, 400).loadGraphic(Paths.image('reimagined'));
		add(reimaginedTxt);
		reimaginedTxt.setGraphicSize(Std.int(reimaginedTxt.width * 0.2));
		reimaginedTxt.updateHitbox();
		reimaginedTxt.antialiasing = ClientPrefs.globalAntialiasing;
		...
		titleText.color = FlxColor.GREEN;
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				FlxTween.tween(reimaginedTxt, {x: reimaginedTxt.x + 750}, 1.35, {ease: FlxEase.quadInOut});

				FlxTween.tween(logoBl, {x: logoBl.x + 50}, 2, {ease: FlxEase.quadInOut, type: PINGPONG});
				FlxTween.tween(logoBl, {y: logoBl.y - 100}, 1, {ease: FlxEase.quadInOut, onComplete: function(twn:FlxTween)
					{
						FlxTween.tween(logoBl, {y: logoBl.y + 600}, 1, {ease: FlxEase.quadInOut});
					}
				});
