...
class PlayState extends MusicBeatState
{
	...
	var funnyFloatyBoys:Array<String> = ['bambi-god2d', 'oppositionexpunged', 'thearchyexpunged', 'tsukaretaexpunged', 'phonophobiaexpunged', 'bamb', 'nimbi', 'conbi'];
	...
	var dayColor:FlxColor = 0xFFFFFFFF;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'FarmDay':
			{
				...
				var bg:FlxSprite = new FlxSprite(-1200, -300).loadGraphic(Paths.image('bambi/day/skye_gapple_reference'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.2, 0.2);
				bg.active = true;
				bg.scale.set(0.7, 0.7);

				var hills:FlxSprite = new FlxSprite(-800, -500).loadGraphic(Paths.image('bambi/day/hills'));
				hills.antialiasing = true;
				hills.scrollFactor.set(0.5, 0.5);
				hills.active = true;
				hills.scale.set(0.7, 0.7);

				var farm:FlxSprite = new FlxSprite(-100, 0).loadGraphic(Paths.image('bambi/day/farm'));
				farm.antialiasing = true;
				farm.scrollFactor.set(0.65, 0.65);
				farm.active = true;
				farm.scale.set(0.7, 0.7);

				var foreground:FlxSprite = new FlxSprite(-300, 350).loadGraphic(Paths.image('bambi/day/foreground'));
				foreground.antialiasing = true;
				foreground.scrollFactor.set(1, 1);
				foreground.active = true;
				foreground.scale.set(0.7, 0.7);
		...
		case 'FarmSunset':
			{
				...
				var bg:FlxSprite = new FlxSprite(-1200, -300).loadGraphic(Paths.image('bambi/sunset/skye_gapple_reference'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.2, 0.2);
				bg.active = true;
		...
		case 'FarmNight': //NO COLOR SET YIPPEE
			{
				defaultCamZoom = 0.7;
				curStage = 'FarmNight';

				var bg:FlxSprite = new FlxSprite(-1200, -300).loadGraphic(Paths.image('bambi/night/skye_gapple_reference'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.2, 0.2);
				bg.active = true;
				bg.scale.set(0.7, 0.7);

				var hills:FlxSprite = new FlxSprite(-800, -500).loadGraphic(Paths.image('bambi/night/hills'));
				hills.antialiasing = true;
				hills.scrollFactor.set(0.5, 0.5);
				hills.active = true;
				hills.scale.set(0.7, 0.7);

				var farm:FlxSprite = new FlxSprite(-100, 0).loadGraphic(Paths.image('bambi/night/farm'));
				farm.antialiasing = true;
				farm.scrollFactor.set(0.65, 0.65);
				farm.active = true;
				farm.scale.set(0.7, 0.7);

				var foreground:FlxSprite = new FlxSprite(-300, 350).loadGraphic(Paths.image('bambi/night/foreground'));
				foreground.antialiasing = true;
				foreground.scrollFactor.set(1, 1);
				foreground.active = true;
				foreground.scale.set(0.7, 0.7);
		...
		msTimeTxt.setFormat(Paths.font('comic.ttf'), 32, 0xFFAC75FF, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		//welcome to purgatory zone act 2

		var composersWatermark:String;
		switch (SONG.song.toLowerCase())
		{
			case 'sweetcorn' | 'cob' | 'divinity':
				composersWatermark = 'joolian';
			
			case 'dismantle':
			    composersWatermark = 'RGBGame';

			case 'tenacity' | 'spamophobia-2' | 'exasperation':
				composersWatermark = 'LiterallyWize';

			case 'ampersand': //idk this could replace tenacity or something | UPDATE: ok it did 
				composersWatermark = 'joolian & Null_y34r';

			case 'resistance':
				composersWatermark = 'Lively Catto | Form by RoundCat';
			
			case 'enmity':
				composersWatermark = 'Villezen | Form by RoundCat';

			case 'annihilation' | 'exuberant':
				composersWatermark = 'Lively Catto';

			case 'tachophobia':
				composersWatermark = 'iplux';

			case 'deceit':
				composersWatermark = 'Sr.Wiliam';

			case 'pleasured':
				composersWatermark = 'BLAZ3R | Form by RoundCat';

			case 'harawotateru':
				composersWatermark = 'ScarX | Form by RoundCat';

			case 'melancholy': 
				composersWatermark = 'AVeryCringeUser/WorldsMostCringe | Character by hortas';
			
			case 'occasions':
				composersWatermark = 'shulginiw';

			case 'blistering' | 'pessimistic':
			    composersWatermark = 'Null_y34r';

			case 'restored':
				composersWatermark = 'PizzaSlizeJ';

			//strident 1 year port augh 
			case 'bash':
				composersWatermark = 'Sinjl & joolian';
			
			case 'sneaky':
				composersWatermark = 'RoundCat | Original Song by Kevin MacLeod';
		...
		add(songTxt);
		songTxt.text = curSong + " | SC: Reimagined " + MainMenuState.curModVer;
		...
		judgementCounter.text = 'Total Notes: ${tnh}\nCRISIS!!: ${perfects}\nAwesome: ${sicks}\nNice: ${goods}\nHmm...: ${bads}\nDamn.: ${shits}\nPhone Breaks: ${songMisses}\nCombo: ${combo}'; // purgatory fixed build-type judgement counter
		judgementCounter.cameras = [camHUD];
	    add(judgementCounter);

		songWatermark = new FlxText(12, 675, 0, "", 8);
		songWatermark.setFormat(Paths.font("comic.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		add(songWatermark);
		songWatermark.text = "Song by " + composersWatermark;
  ...
  public function clearShaderFromCamera(cam:String){
	...
	{
		if(ratingName == '?') {
			scoreTxt.text = 'NPS: 0 (Max 0) | Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' | Average: ? | Accuracy: 0% [N/A]' ;
		} else {
			scoreTxt.text = 'NPS: ' + nps + ' (Max ' + maxnps + ') | Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' | Average: ' + Math.round(averageMs) + 'ms' + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%' + ' | ' + ratingName + ' [' + ratingFC + ']';
		}
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (!isStoryMode && !skipArrowStartTween)
			{
				...
				babyArrow.alpha = 0;
				babyArrow.angle = -90;
				FlxTween.tween(babyArrow, {/*y: babyArrow.y + 10,*/ alpha: targetAlpha, angle: 0}, 1, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i)});
			}
			else
			{
	...
	{
		if (SONG.song.toLowerCase() == 'kernhell')
		{
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
						NewStoryState.weekCompleted.set(WeekData.weeksList[storyWeek], true);
						...
						FlxG.save.data.weekCompleted = NewStoryState.weekCompleted;
						FlxG.save.flush();
					}
	...
	override function beatHit()
	{
		...
		if (ClientPrefs.iconbops == "OS" || ClientPrefs.iconbops == "Psych" || ClientPrefs.iconbops == "Strident") { //you do not have an option :trollge:
			if (dancingLeft){
		...
		/*switch (SONG.song.toLowerCase())
		{
			...
			{
				case 228:
					FlxTween.color(dad, 0.01, dad.color, dayColor);
					FlxTween.color(boyfriend, 0.01, boyfriend.color, dayColor);
					FlxTween.color(gf, 0.01, gf.color, dayColor);
				case 416:
					FlxTween.color(dad, 0.01, dad.color, nightColor);
					FlxTween.color(boyfriend, 0.01, boyfriend.color, nightColor);
					FlxTween.color(gf, 0.01, gf.color, nightColor);
			}
		}*/
