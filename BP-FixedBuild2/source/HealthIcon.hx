...
class HealthIcon extends FlxSprite
{
		...
		if(this.char != char) {
			switch(char) {
				...
				default:
					...
					if (file.width == 300) { // ICONS FILES THAT HAVE NO WINNING ICON
						...
						animation.play(char);
						this.isAnim = false;
					} else if (file.width == 524) { // SHOWDOWN ICON
						loadGraphic(file); //Load stupidly first for getting the file size
						loadGraphic(file, true, Math.floor(width / 3), Math.floor(height)); //Then load it fr
						iconOffsets[0] = (width - 182) / 2;
						iconOffsets[1] = (width - 182) / 2;
						iconOffsets[2] = (width - 182) / 2;
						updateHitbox();

						animation.add(char, [0, 1, 2], 0, false, isPlayer);
						animation.play(char);
						this.isAnim = false;
					} else { // ICON FILES WITH WINNING ICONS AS DEFAULT
						loadGraphic(file); //Load stupidly first for getting the file size
						loadGraphic(file, true, Math.floor(width / 3), Math.floor(height)); //Then load it fr
						iconOffsets[0] = (width - 150) / 2;
						iconOffsets[1] = (width - 150) / 2;
	...
	public function updateHitboxPE() // Was used for customizable HUD layouts, dunno if I'll add it back here though - FrogB
	{
		super.updateHitbox();
