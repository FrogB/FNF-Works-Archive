...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		spikes = new FlxBackdrop(Paths.image('uilol/spikeys'), 1, 0, true, false);
...
/*class Section2Substate extends MusicBeatSubstate
{
	...
	public function new() {
	    ...
	    var bgSub:FlxSprite = new FlxSprite(-80).loadGraphic(MainMenuState.randomizeBG());
		bgSub.updateHitbox();
		bgSub.screenCenter();
		bgSub.antialiasing = ClientPrefs.globalAntialiasing;
		add(bgSub);
		bgSub.color = 0xFF3F0000;

		
		var checkerSub:FlxBackdrop;
		checkerSub = new FlxBackdrop(Paths.image('uilol/check'),XY);
		...
		var okSub:FlxSprite = new FlxSprite().loadGraphic(Paths.image('uilol/glow'));
		...
		var lineSub:FlxSprite = new FlxSprite().loadGraphic(Paths.image('uilol/line'));
		...
		sliderSub = new FlxBackdrop(Paths.image('uilol/hahaslider'), X);
		...
		spikesSub = new FlxBackdrop(Paths.image('uilol/spikeys'),X);
