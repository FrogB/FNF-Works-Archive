...
class BlockedGlitchEffect2
{
	public var shader(default, null):BlockedGlitchShader2 = new BlockedGlitchShader2();
...
class BlockedGlitchShader2 extends FlxShader
{
	// https://www.shadertoy.com/view/MlVSD3
	@:glFragmentSource('
    #pragma header
...
// oof told me to test this 
class BetterChromaticAberrationShader extends FlxShader
{
	@:glFragmentSource('
	#pragma header
	...
	vec4 BetterChromaticAberrationShader(sampler2D tex, in vec2 uv) 
	{
	...
	void main(){
		vec2 uv = openfl_TextureCoordv;
		vec4 col = BetterChromaticAberrationShader(bitmap, uv);
...
class BetterChromaticAberrationEffect extends Effect
{
	public var shader:BetterChromaticAberrationShader = new BetterChromaticAberrationShader();

	public var amount(default, set):Float = 0;

	public function new(amount:Float = 0.00):Void
	{
		this.amount = amount;
	}
	...
	{
		amount = v;
		shader.rChannel.value = [0.3 * v];
		shader.bChannel.value = [0.15 * v];
		shader.gChannel.value = [0.075 * v];
		shader.aChannel.value = [0.15 * v];
		return v;
	}
}
